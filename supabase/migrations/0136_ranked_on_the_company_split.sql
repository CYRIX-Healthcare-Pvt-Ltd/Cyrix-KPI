-- =====================================================================
-- Cyrix KPI  ·  0136  ·  Ranked on the company split
--
-- The ranking weighed the two bands 60/40. The KPI itself is 80/20 — 80
-- marks of job role, 20 of core values — and being ranked on a different
-- split from the one everybody is scored on is a question nobody could
-- answer well. So the ranking now uses the company's own split.
--
-- What it means, said plainly: one job-role band is 0.8, and the whole
-- core-values range from 1 to 5 is 4 x 0.2 = 0.8. So a higher job band
-- can at best be equalled by core values, never beaten, and the tie-break
-- is the job band. Job role decides the order; core values separate
-- people inside it. At 60/40 a strong core could overturn a job band.
--
-- This moves ranks. Every screen that ranks reads the same figure, and
-- the manager ranking averages it, so those move with it.
-- =====================================================================
  ...
  combined as (
    select
      ...
      case
        ...
        else 0.8 * kpi_rating(sp.job_pct) + 0.2 * kpi_rating(sp.core_pct)
      end as rank_v
