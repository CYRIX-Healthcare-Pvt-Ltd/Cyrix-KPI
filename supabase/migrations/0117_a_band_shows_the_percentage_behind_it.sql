-- =====================================================================
-- Cyrix KPI  ·  0117  ·  A band shows the percentage behind it
--
-- Asked by the SW admin, who could not answer it: E639 averages 93.0 and
-- sits 60th, E1427 averages 89.4 and sits 38th.
--
--   E1427  job 90.2% -> band 5   core 86.4% -> band 4   =  4.60
--   E639   job 98.5% -> band 5   core 71.2% -> band 3   =  4.20
--
-- The total is 80 marks of job role against 20 of core values, while the
-- ranking weighs the two BANDS 60/40 — so a strong job role cannot carry
-- a core-values 71.2%, which is a 3. Obvious once the halves are on
-- screen, and impossible to explain while the tile says only "3 of 5".
--
-- The bands were already returned; the percentages behind them were not.
-- Both are computed in the function already, so this returns them.
--
-- The body below is the live definition read back with pg_get_functiondef,
-- with three splices and nothing retyped: two columns on the RETURNS
-- TABLE, the two percentages carried through the `ranked` CTE, and the
-- two returned at the end. Same arithmetic, same ordering, same rows;
-- appended columns are additive for every existing caller.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- Self-test: every band comes back with the percentage it was computed
-- from, and the two still agree with the slab.
-- ---------------------------------------------------------------------
do $$
declare
  bad integer;
...
begin
  select count(*), count(*) filter (
      where (k.job_band is not null and k.job_pct is null)
         or (k.core_band is not null and k.core_pct is null)
         or (k.job_pct is not null and k.job_band is distinct from kpi_rating(k.job_pct))
         or (k.core_pct is not null and k.core_band is distinct from kpi_rating(k.core_pct)))
    into seen, bad
  from (
    select e.id
    from employees e
    where e.is_active
      and exists (select 1 from kpi_submissions s
  ...
  if bad > 0 then
    raise exception '% of % ranking row(s) whose band does not follow its percentage', bad, seen;
  end if;

  raise notice '0117 self-test passed (% rows: every band carries the percentage it came from)', seen;
end $$;
