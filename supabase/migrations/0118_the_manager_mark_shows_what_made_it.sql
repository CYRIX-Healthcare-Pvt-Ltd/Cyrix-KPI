-- =====================================================================
-- Cyrix KPI  ·  0118  ·  The manager mark shows what made it
--
-- The same question 0117 answered for the two employee ranks, asked of
-- the third: "Team scoring rank 36th of 172" with a panel that lists
-- 70 / 20 / 10 and then five figures in days, none of which is the mark
-- the position was sorted on.
--
-- What the panel could not say, and now can:
--
--   * the mark itself. mgr_overall has been returned since 0096 and has
--     never been shown on any screen.
--   * what the 70 was read off. The team average band is computed inside
--     the function and thrown away; a manager could see every reportee's
--     score and still not reproduce their own number.
--   * that the whole mark is currently that one part. Neither turnaround
--     has anything to measure yet -- the TAT policy starts from Sep-2026
--     and no month has been submitted or scored inside it -- so all 172
--     managers are ranked on the team band alone, reweighted to 100%.
--     Every one of them is reading 70 / 20 / 10 and none of it is true
--     for them today.
--
-- Both new columns already exist inside the function; this returns them.
-- Appended, so every existing caller is unaffected.
--
-- The body below is the live definition read back with pg_get_functiondef,
-- with four splices and nothing retyped: two columns on the RETURNS
-- TABLE, a count added beside the average in team_band, both carried
-- through mgr_marked, and both returned at the end.
-- =====================================================================

-- Dropped and recreated rather than replaced: the two new columns change
-- the return type, and postgres refuses CREATE OR REPLACE for that. The
-- grants go with the drop, so they are restored below exactly as they
-- were -- execute to authenticated and service_role, owner postgres.
...
-- ---------------------------------------------------------------------
-- Self-test: the band comes back with the count behind it, on the slab
-- it is defined on, and -- while both turnarounds are unmeasurable --
-- it is the whole of the mark.
-- ---------------------------------------------------------------------
do $$
declare
  seen    integer;
...
begin
  select
    ...
    count(*) filter (
      where (k.mgr_team_band is not null
             and (k.mgr_team_band < 1 or k.mgr_team_band > 5
                  or coalesce(k.mgr_team_scored, 0) < 1))
         or (k.mgr_team_band is null and k.mgr_team_scored is not null)
         -- Nothing measurable at all must stay null rather than becoming
         -- a zero somebody is then ranked on.
         or (k.mgr_team_band is null and k.submit_tat is null
             and k.completion_tat is null and k.mgr_overall is not null)),
    count(*) filter (
      where k.mgr_team_band is not null
        and k.submit_tat is null and k.completion_tat is null
        -- The band, reweighted to the whole mark, IS the mark. 0.3 of
        -- slack for the two roundings: the band to 2 dp here, the mark
        -- to 1 dp in the function.
        and abs(k.mgr_overall - 100 * (k.mgr_team_band - 1) / 4) > 0.3)
    into seen, bad, lonely
  from (
    select e.id
    from employees e
    where e.is_active
  ...
  if bad > 0 then
    raise exception '% of % manager row(s) with a band that does not describe itself', bad, seen;
  end if;
  if lonely > 0 then
    raise exception '% of % manager row(s) whose lone band does not add up to the mark', lonely, seen;
  end if;

  raise notice '0118 self-test passed (% managers: the band, its count, and the mark it makes)', seen;
end $$;
