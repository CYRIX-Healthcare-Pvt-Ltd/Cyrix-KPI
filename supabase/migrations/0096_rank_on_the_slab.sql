-- =====================================================================
-- Cyrix KPI  ·  0096  ·  Rank on the slab
--
-- 0095 took the 120% ceiling off, which was the only thing stopping one
-- tripled target from carrying a year. Ranking has run on the raw
-- average since 0026, so without a ceiling somebody outstanding on one
-- KRA and poor on every other one would now top the list.
--
-- The ceiling was the wrong tool for that: it answered a ranking
-- question by falsifying a score. This answers it where it lives. On the
-- slab, 190% and 95% are both simply a 5, so the extraordinary row stops
-- compounding and the score is free to say what actually happened.
--
-- Two rules, both from management after the demo.
--
--   Employee   0.6 x job-role band + 0.4 x core-values band
--   Manager   (0.1 x team submission TAT
--            + 0.3 x manager completion TAT
--            + 0.6 x the team's average band)
--            x the share of the team's year actually scored
--
-- The employee ratio was chosen with the trade-off stated: at 60/40 core
-- values can overturn a job-role gap, so job 4 / core 5 (4.4) finishes
-- above job 5 / core 1 (3.4). Ranking on the job band alone with core
-- breaking ties was the alternative and was declined.
--
-- ESMS is deliberately not in the employee figure. Management named two
-- bands and ESMS is neither; it still counts toward the score itself.
-- Worth knowing rather than discovering: an ESMS carrier's ESMS
-- performance moves their total and not their rank.
--
-- src/lib/rating.ts mirrors all of this and is tested against the same
-- cases. The two must agree.
-- =====================================================================

-- ---------------------------------------------------------------------
-- The slab, in SQL.
-- ---------------------------------------------------------------------
create or replace function public.kpi_rating(p_pct numeric)
returns int
language sql
immutable
as $$
  select case
    when p_pct is null then null
    -- The one bound inclusive at the bottom: ">=50 and <=60". 50 exactly
    -- is a 2, because half marks does not belong with nothing.
    when p_pct >= 50 and p_pct <= 60 then 2
    when p_pct > 90 then 5
    when p_pct > 80 then 4
    when p_pct > 60 then 3
    else 1
  end
$$;

comment on function public.kpi_rating(numeric) is
  'Management''s 1-5 slab. Bands close at the top: 90 is a 4, 80 is a 3, '
  '60 is a 2. Mirrored by ratingFor in src/lib/rating.ts.';

-- ---------------------------------------------------------------------
-- Days to a 0-1 mark. Fewer days is higher.
--
-- Measured against the company's own allowance rather than a constant,
-- so changing the TAT policy moves the ranking with it. Using the whole
-- allowance keeps half the marks; twice it keeps none. Full marks
-- anywhere inside the allowance cannot rank at all, because nearly
-- everybody is inside it -- and separating a manager who scores in two
-- days from one who takes five is the entire job of the component.
-- ---------------------------------------------------------------------
create or replace function public.tat_mark(p_days numeric, p_allowance numeric)
returns numeric
language sql
immutable
as $$
  select case
    when p_days is null or p_allowance is null or p_allowance <= 0 then null
    else greatest(0, least(1, 1 - p_days / (p_allowance * 2)))
  end
$$;

comment on function public.tat_mark(numeric, numeric) is
  'Turnaround days to a 0-1 mark against the policy allowance. '
  'Mirrored by tatScore in src/lib/rating.ts (LATE_HORIZON = 2).';


-- ---------------------------------------------------------------------
-- kpi_ranking, on the slab.
--
-- Dropped and recreated rather than replaced, because the return type
-- gains four columns: the two bands, the combined figure they produce,
-- and the manager's overall mark. Screens that show a rank should be
-- able to show its working -- a position with no way to see how it was
-- reached is the kind of number people stop believing.
-- ---------------------------------------------------------------------
drop function if exists public.kpi_ranking(uuid, text);

create function public.kpi_ranking(
  p_employee_id uuid default null,
  p_financial_year text default null
)
returns table (
  employee_id uuid, financial_year text, score numeric,
  team_rank int, team_of int, org_rank int, org_of int, team_size int,
  mgr_rank int, mgr_of int, completion_pct numeric,
  due_months int, scored_months int,
  submit_tat numeric, completion_tat numeric, pending_tat numeric,
  submit_delay numeric, completion_delay numeric, pending_delay numeric,
  tm_grace_days int, mgr_grace_days int, tat_starts_from date,
  -- New in 0096.
  job_band int, core_band int, rank_value numeric, mgr_overall numeric
)
language plpgsql
stable security definer
set search_path to 'public'
as $function$
declare
  target uuid := coalesce(p_employee_id, current_employee_id());
  fy     text := coalesce(p_financial_year,
                          (select code from financial_years where is_current));
  pol    jsonb := coalesce(
                    (select value from app_settings where key = 'tat_policy'),
                    '{}'::jsonb);
  tm_days  numeric := coalesce((pol->>'tm_grace_days')::numeric, 3);
  mgr_days numeric := coalesce((pol->>'manager_grace_days')::numeric, 5);
begin
  if target is null then
    return;
  end if;

  if not (target = current_employee_id()
          or manages_employee(target)
          or is_hr_admin()) then
    raise exception 'You can only see your own ranking';
  end if;

  return query
  with scored_people as (
    select
      s.employee_id                       as emp,
      e.reporting_manager_id              as mgr,
      round(avg(s.final_total_score), 4)  as avg_score,
      -- Each half as a share of its OWN weightage, which is what the
      -- slab is defined on. The weights differ per person: core values
      -- is 20 normally and 15 for anybody carrying ESMS, so dividing by
      -- a constant would rate the two groups on different scales.
      avg(100.0 * s.final_job_role_score / nullif(a.job_role_weight, 0))    as job_pct,
      avg(100.0 * s.final_core_score     / nullif(a.core_values_weight, 0)) as core_pct
    from kpi_submissions s
    join employees e on e.id = s.employee_id and e.is_active
    left join kpi_assignments a
      on a.employee_id = s.employee_id and a.financial_year = s.financial_year
    where s.financial_year = fy
      and s.status in ('scored', 'finalized')
      and s.final_total_score is not null
    group by s.employee_id, e.reporting_manager_id
  ),
  combined as (
    select
      sp.*,
      kpi_rating(sp.job_pct)  as job_b,
      kpi_rating(sp.core_pct) as core_b,
      -- One half scored and not the other stands on its own rather than
      -- being averaged against a zero nobody awarded.
      case
        when kpi_rating(sp.job_pct) is null and kpi_rating(sp.core_pct) is null
          then null
        when kpi_rating(sp.job_pct) is null  then kpi_rating(sp.core_pct)::numeric
        when kpi_rating(sp.core_pct) is null then kpi_rating(sp.job_pct)::numeric
        else 0.6 * kpi_rating(sp.job_pct) + 0.4 * kpi_rating(sp.core_pct)
      end as rank_v
    from scored_people sp
  ),
  ranked as (
    select
      c.emp,
      c.avg_score,
      c.job_b,
      c.core_b,
      c.rank_v,
      -- The band first, then the job band, then the score itself. The
      -- slab is coarse on purpose, so without a final tie-break a team
      -- of five 4s would all be joint first.
      rank() over (partition by c.mgr
                   order by c.rank_v desc nulls last, c.job_b desc nulls last,
                            c.avg_score desc)                as t_rank,
      count(*) over (partition by c.mgr)                     as t_of,
      rank() over (order by c.rank_v desc nulls last, c.job_b desc nulls last,
                            c.avg_score desc)                as o_rank,
      count(*) over ()                                       as o_of
    from combined c
  ),
  by_manager as (
    select
      r.manager_id                                                    as mgr_id,
      count(*)::int                                                   as n_due,
      count(*) filter (
        where r.status in ('scored','finalized'))::int                as n_done,
      round(100.0 * count(*) filter (
        where r.status in ('scored','finalized')) / count(*), 1)      as pct,
      round(avg(r.submit_tat_days) filter (where r.counts_for_tat), 1)     as sub_tat,
      round(avg(r.completion_tat_days) filter (where r.counts_for_tat), 1) as comp_tat,
      round(avg(r.pending_tat_days) filter (where r.counts_for_tat), 1)    as pend_tat,
      round(avg(r.submit_delay_days), 1)                              as sub_late,
      round(avg(r.completion_delay_days), 1)                          as comp_late,
      round(avg(r.pending_delay_days), 1)                             as pend_late
    from v_kpi_report_rows r
    where r.financial_year = fy
      and r.period_month < date_trunc('month', current_date)::date
    group by r.manager_id
  ),
  team_band as (
    select c.mgr as mgr_id, avg(c.rank_v) as team_v
    from combined c
    where c.rank_v is not null
    group by c.mgr
  ),
  mgr_marked as (
    select
      b.*,
      tat_mark(b.sub_tat, tm_days)   as sub_mark,
      tat_mark(b.comp_tat, mgr_days) as comp_mark,
      -- How much of the team's year has actually been scored.
      --
      -- Not in the ratio management first gave, and the dry run showed
      -- why it has to be: without it a manager who had scored 1.2% of
      -- their team's months ranked FIRST, above one who had done 80%,
      -- because turnaround was the only thing measured and being quick
      -- about almost nothing beat being thorough. Speed on work you have
      -- not done is not a performance.
      case when b.pct is null then null else b.pct / 100.0 end as done_mark,
      -- The 1-5 slab onto 0-1, so it sits beside the two TAT marks.
      case when t.team_v is null then null else (t.team_v - 1) / 4 end as team_mark
    from by_manager b
    left join team_band t on t.mgr_id = b.mgr_id
  ),
  mgr_scored as (
    select
      m.*,
      -- Components that cannot be measured are dropped and the rest
      -- reweighted among themselves. A manager whose team has submitted
      -- nothing has no completion TAT, and scoring that as zero would
      -- rank them below one who scored everything late -- an absence of
      -- evidence read as a failure.
      -- Quality times coverage, not quality plus coverage.
      --
      -- Management's ratio is kept exactly as given for the quality half
      -- -- 20 submission TAT, 40 completion TAT, 40 team band -- and then
      -- multiplied by the share of the team's year actually scored.
      --
      -- Adding completion as a fourth weight was tried first and does
      -- not work, and the dry run is why: with an additive formula the
      -- other components are still computed from whatever tiny sample
      -- exists, so a manager who had scored ONE of eighty-one months
      -- came first on turnaround and a team band drawn from that single
      -- month. No set of weights fixes that, because the problem is not
      -- how much completion counts -- it is that the other three numbers
      -- mean nothing without it. A fortnight's turnaround on 1% of your
      -- team is not a fast manager, it is an unmeasured one.
      --
      -- So coverage scales the whole figure: 80% of the work done well
      -- beats 1% done quickly, and a manager at half completion carries
      -- half their quality into the ranking.
      case
        when (case when m.sub_mark  is null then 0 else 0.1 end)
           + (case when m.comp_mark is null then 0 else 0.3 end)
           + (case when m.team_mark is null then 0 else 0.6 end) = 0
        then null
        else round(100 * coalesce(m.done_mark, 0) * (
               coalesce(m.sub_mark  * 0.1, 0)
             + coalesce(m.comp_mark * 0.3, 0)
             + coalesce(m.team_mark * 0.6, 0)
             ) / (
               (case when m.sub_mark  is null then 0 else 0.1 end)
             + (case when m.comp_mark is null then 0 else 0.3 end)
             + (case when m.team_mark is null then 0 else 0.6 end)
             ), 1)
      end as overall
    from mgr_marked m
  ),
  mgr_ranked as (
    select
      ms.*,
      rank() over (order by ms.overall desc nulls last) as m_rank,
      count(*) over ()                                  as m_of
    from mgr_scored ms
  )
  select
    target,
    fy,
    round(r.avg_score, 2),
    r.t_rank::int,
    r.t_of::int,
    r.o_rank::int,
    r.o_of::int,
    (select count(*)::int
     from employees p
     where p.is_active
       and p.reporting_manager_id =
           (select reporting_manager_id from employees where id = target)),
    m.m_rank::int,
    m.m_of::int,
    m.pct,
    m.n_due,
    m.n_done,
    m.sub_tat,
    m.comp_tat,
    m.pend_tat,
    m.sub_late,
    m.comp_late,
    m.pend_late,
    tm_days::int,
    mgr_days::int,
    nullif(pol->>'starts_from', '')::date,
    r.job_b::int,
    r.core_b::int,
    round(r.rank_v, 2),
    m.overall
  from (select 1) one
  left join ranked     r on r.emp = target
  left join mgr_ranked m on m.mgr_id = target;
end $function$;

revoke all on function public.kpi_ranking(uuid, text) from public, anon;
grant execute on function public.kpi_ranking(uuid, text) to authenticated;

notify pgrst, 'reload schema';


-- ---------------------------------------------------------------------
-- Self-test.
-- ---------------------------------------------------------------------
do $$
declare
  n numeric;
begin
  -- The slab, at every boundary anybody will argue about.
  if kpi_rating(90)   <> 4 then raise exception '90 should be a 4'; end if;
  if kpi_rating(90.1) <> 5 then raise exception '90.1 should be a 5'; end if;
  if kpi_rating(80)   <> 3 then raise exception '80 should be a 3'; end if;
  if kpi_rating(60)   <> 2 then raise exception '60 should be a 2'; end if;
  if kpi_rating(50)   <> 2 then raise exception '50 should be a 2'; end if;
  if kpi_rating(49.9) <> 1 then raise exception '49.9 should be a 1'; end if;
  if kpi_rating(190)  <> 5 then raise exception 'over-achievement is just a 5'; end if;
  if kpi_rating(null) is not null then raise exception 'null stays null'; end if;

  -- The TAT mark, against an allowance rather than a constant.
  if tat_mark(0, 5)  <> 1   then raise exception 'instant should be full marks'; end if;
  if tat_mark(5, 5)  <> 0.5 then raise exception 'the whole allowance is half marks'; end if;
  if tat_mark(10, 5) <> 0   then raise exception 'twice the allowance is none'; end if;
  if tat_mark(99, 5) <> 0   then raise exception 'never below zero'; end if;
  if tat_mark(5, 10) <> 0.75 then raise exception 'it must move with the policy'; end if;

  -- The ratio, on management's own example: A is job 4 / core 4, B is
  -- job 5 / core 3, and B must come first.
  if not (0.6 * 5 + 0.4 * 3 > 0.6 * 4 + 0.4 * 4) then
    raise exception 'B should outrank A';
  end if;

  -- And the function still answers for a real person.
  select count(*) into n from kpi_ranking(
    (select id from employees where ecode = 'E1427'), '2026-27');
  if n <> 1 then
    raise exception 'kpi_ranking returned % rows, expected 1', n;
  end if;

  raise notice '0096 self-test passed (slab, TAT marks, ratio, and a live row)';
end $$;
